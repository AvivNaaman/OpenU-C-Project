...
int process_code(char *line, int i) {
    ...
    if (curr_opcode <= STOP_OP && curr_opcode >= RTS_OP) {
        ...
        /* if opcode is 0*/
        if (MOV_OP == curr_opcode) {
            if (!((IMMEDIATE == type1 || DIRECT == type1 || REG == type1) &&
                  (DIRECT == type2 || REG == type2))) {
                printf("Error: illegal addressing type");
                return TRUE;
            }
        }
            /*if opcode is 1*/
        else if (CMP_OP == curr_opcode) {
            if (!((IMMEDIATE == type1 || DIRECT == type1 || REG == type1) &&
                  (IMMEDIATE == type1 || DIRECT == type2 || REG == type2))) {
                printf("Error: illegal addressing type");
                return TRUE;
            }
        }
            /*if opcode is 2*/
        else if (ADD_OP == curr_opcode) {
            if (!((IMMEDIATE == type1 || DIRECT == type1 || REG == type1) &&
                  (DIRECT == type2 || REG == type2))) {
                printf("Error: illegal addressing type");
                return TRUE;
